...
void setup() {
  ...
  initial_position = starting_point(prev_state);
}
...
void loop() {
  ...
  if (state != prev_state) {
    ...
    failure = backwards_with_check(state, minus_counter, initial_position);
    if (failure == true) {
      Serial.println(error_text);
      // Consider Ending Program Here (Return Exit Failure arduino equivalent)
    }
...
int starting_point(int prev_state[BITS]) {
  int j = 0;
  while (prev_state[1] != EXPECTED[j][0] && prev_state[0] != EXPECTED[j][1])
    j++;
  return j;
}


bool backwards_with_check (int state[BITS], int & minus_counter, int initial_position) {
